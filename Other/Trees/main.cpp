...
int main(int argc, char* argv[]) {
    ...
    QObject::connect(&insertButton, &QPushButton::clicked, [&binaryTreeWidget]() {
        bool ok;
        int key = QInputDialog::getInt(nullptr, "Insert Node", "Enter key:", 0, INT_MIN, INT_MAX, 1, &ok);
        if (ok) {
    ...
    QObject::connect(&deleteButton, &QPushButton::clicked, [&binaryTreeWidget]() {
        bool ok;
        int key = QInputDialog::getInt(nullptr, "Delete Node", "Enter key:", 0, INT_MIN, INT_MAX, 1, &ok);
        if (ok) {
